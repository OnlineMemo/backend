...
public class UserMemoServiceImpl implements UserMemoService {
    ...
    public void inviteUsersToMemo(Long memoId, List<Long> userIdList) {
        ...
        if(!userMemoRepository.existsByUser_IdAndMemo_Id(loginUserId, memoId)) {  // 애초에 초대할 권한(메모에 포함되지않은 사용자인 경우)이 없는 경우
            throw new Exception404.NoSuchUserMemo(String.format("userId = %d, memoId = %d", loginUserId, memoId));
        }

        // 강제 Eager 조회 (N+1 문제 해결)
        Memo memo = memoRepository.findByIdWithEager(memoId).orElseThrow(
                () -> new Exception404.NoSuchUser(String.format("memoId = %d", memoId)));

        // 공동메모에 이미 참여중인 사용자들
        Set<Long> existUserIdSet = memo.getUserMemoList().stream()  // Memo.Usermemo (N+1 쿼리 발생)
                .map(userMemo -> userMemo.getUser().getId())  // Memo.Usermemo.user (N+1 쿼리 발생)
